...
def process_geos(dir, top, left, bottom, right):
    dir_listing = os.listdir(dir)
    df = None
    for file in dir_listing:
        if ".png" in file:
            ...
            if df is None:
                ...
                df_temp = pd.DataFrame(cropped, index=long_range, columns=lat_range)
                df_temp = pd.melt(df_temp.reset_index(), id_vars='index')
                df_temp.columns = ['Lat', 'Long', feature]
                df_temp['Date'] = pd.to_datetime(datestamp, format="%d%b%Y")
                df = pd.concat([df, df_temp])
    return df
...
if __name__ == "__main__":
    ...
    dir_geo = '/Users/josh/Google Drive/Georgia Tech Notes/Capstone/data/geo_data'
    top = 26.405982971191
